...
import './libraries/TransferHelperExternal.sol';
...
// XXX: TransferHelper library used as external compared to UniswapV2Router02
contract UniswapV2Router03 is IUniswapV2Router02 {
    ...
    function removeLiquidityETHSupportingFeeOnTransferTokens(
        ...
        _safeTransfer(token, to, IERC20Uniswap(token).balanceOf(address(this)));
        IWETH(WETH).withdraw(amountETH);
        _safeTransferETH(to, amountETH);
    }
    ...
    // **** INTERNAL FUNCTIONS INTERACTING WITH TRANSFER_HELPER_EXTERNAL LIBRARY ****
    function _safeTransfer(address token, address to, uint value) internal {
        TransferHelperExternal.safeTransfer(token, to, value);
    }

    function _safeTransferFrom(address token, address from, address to, uint value) internal {
        TransferHelperExternal.safeTransferFrom(token, from, to, value);
    }

    function _safeTransferETH(address to, uint value) internal {
        TransferHelperExternal.safeTransferETH(to, value);
    }
}
